...
import { EVENT_SPORT_QUERY } from "../api/query";
import { DEL_SPORT_EVENT } from "../api/mutation";
...
export default function Sport(props) {
  ...
  const { loading, error, data, refetch } = useQuery(EVENT_SPORT_QUERY, {
    pollInterval: 200
  });
  const [delEventSociety] = useMutation(DEL_SPORT_EVENT);
  if (loading) {
    return (
