...
import { EVENT_POLITIC_QUERY } from "../api/query";
import { DEL_POLITIC_EVENT } from "../api/mutation";
...
export default function Politics(props) {
  ...
  const { loading, error, data, refetch } = useQuery(EVENT_POLITIC_QUERY, {
    pollInterval: 200
  });
  const [delEventSociety] = useMutation(DEL_POLITIC_EVENT);
  if (loading) {
    return (
